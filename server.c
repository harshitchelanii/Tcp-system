...
void *handle_client(void *arg) {
  ...
  clients[client_index].username[sizeof(clients[client_index].username) - 1] =
      '\0';
        ...
        *space = '\0';
        ...
        if (target_index == -1) {
          ...
          snprintf(private_message, sizeof(private_message),
                   "[WAYP] 🔒 %s: %s\n", clients[client_index].username,
                   msg_content);
          send(clients[target_index].socket, private_message,
  ...
  snprintf(system_message, sizeof(system_message), "[WAYP] %s left the chat.\n",
           clients[client_index].username);
